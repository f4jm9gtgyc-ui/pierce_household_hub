...
create table if not exists nursery_checklist (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references auth.users(id) on delete cascade not null,
...
alter table nursery_checklist enable row level security;
...
create policy "Users can read own checklist" on nursery_checklist for select using (auth.uid() = user_id);
create policy "Users can insert own checklist" on nursery_checklist for insert with check (auth.uid() = user_id);
create policy "Users can update own checklist" on nursery_checklist for update using (auth.uid() = user_id);
create policy "Users can delete own checklist" on nursery_checklist for delete using (auth.uid() = user_id);

create policy "Users can read own expenses" on budget_expenses for select using (auth.uid() = user_id);
...
create policy "Users can read own budget settings" on budget_settings for select using (auth.uid() = user_id);
create policy "Users can insert own budget settings" on budget_settings for insert with check (auth.uid() = user_id);
create policy "Users can update own budget settings" on budget_settings for update using (auth.uid() = user_id);
create policy "Users can delete own budget settings" on budget_settings for delete using (auth.uid() = user_id);
